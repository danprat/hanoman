/* AppUI.jsx — small interaction primitives used by the prototype so
   every action has real feedback: a Modal shell, a Toast, and a couple
   of form-field wrappers styled to the design system. */
const { Icon: UIcon, Button: UBtn } = window.HanomanDesignSystem_c639ad;

/* ---- Toast: transient confirmation, bottom-center ---- */
function useToast() {
  const [toast, setToast] = React.useState(null);
  const tRef = React.useRef(null);
  const show = React.useCallback((message, tone = "ok", icon) => {
...
function Toast({ toast }) {
  ...
  return (
    <div key={toast.k} style={{
      ...
      <UIcon name={icon} size={16} color={color} />
...
/* ---- Modal shell ---- */
function Modal({ open, title, eyebrow, icon, onClose, footer, width = 560, children }) {
  React.useEffect(() => {
    if (!open) return;
...
/* ---- textarea (DS ships no Textarea; matched to the Input well) ---- */
function HnTextarea({ value, onChange, rows = 3, placeholder, mono = false }) {
...
function pageWindow(page, pageCount) {
  ...
  const near = (n) => n === 1 || n === pageCount || Math.abs(n - page) <= 1;
  let last = 0;
  for (let n = 1; n <= pageCount; n++) {
    if (near(n)) { if (last && n - last > 1) push("…"); push(n); last = n; }
  }
  return out;
}

function Pager({ page, pageCount, total, from, to, onPage, unit = "item" }) {
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
        <PagerBtn onClick={() => onPage(page - 1)} disabled={page <= 1} aria="Sebelumnya">
          <UIcon name="chevron-left" size={15} />
        </PagerBtn>
        {pageWindow(page, pageCount).map((n, i) => n === "…"
          ...
          : <PagerBtn key={n} onClick={() => onPage(n)} active={n === page} aria={"Halaman " + n}>{n}</PagerBtn>)}
        <PagerBtn onClick={() => onPage(page + 1)} disabled={page >= pageCount} aria="Berikutnya">
          <UIcon name="chevron-right" size={15} />
        </PagerBtn>
      </div>
    </div>
  );
...
Object.assign(window, { useToast, Toast, Modal, Field, HnTextarea, usePaged, Pager });
