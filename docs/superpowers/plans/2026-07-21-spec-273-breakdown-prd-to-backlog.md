...
- [x] **Step 1: Write the failing test** — buat `shared/test/breakdown.test.ts`:
...
- [x] **Step 3a: Edit `shared/src/dto.ts`** — tambah `"breakdown"` ke `zFlow`:
...
- [x] **Step 3b: Edit `shared/src/dto.ts`** — tambah varian breakdown ke `zTerminalSession` (setelah varian prd, baris ~126):
...
- [x] **Step 3c: Edit `shared/src/api.ts`** — setelah `prdFile` (baris ~26) tambah:
...
- [x] **Step 5: Typecheck shared + commit**
...
- [x] **Step 1: Write the failing test** — tambah di `runner/test/prompt.test.ts` (impor `startBreakdownPrompt` di baris import atas):
...
- [x] **Step 3a: Edit `runner/src/types.ts`** — perbarui `Flow` dan tambah tipe:
...
- [x] **Step 3b: Edit `runner/src/prompt.ts`** — impor `BreakdownPrd`, tambah pipeline, dan fungsi prompt.
...
- [x] **Step 1: Write the failing test** — `server/test/project-breakdowns.test.ts`:
...
- [x] **Step 3: Create `server/src/services/project-breakdowns.ts`:**
...
- [x] **Step 1: Write the failing test** — tambah di `server/test/docs.route.test.ts` (di dalam file, blok describe baru di bawah). `dir` sudah di-seed di `beforeEach` global; tambah manifest ke seed itu terlebih dulu:
...
- [x] **Step 3: Edit `server/src/routes/docs.ts`** — impor + route. Ubah baris import:
```ts
...
- [x] **Step 1: Write the failing test** — `server/test/specs-batch.route.test.ts`:
...
- [x] **Step 3: Edit `server/src/routes/specs.ts`** — impor `zBatchCreateSpec` di baris import shared:
```ts
...
- [x] **Step 1: Write the failing test** — tambah di `server/test/terminal.route.test.ts` (blok describe baru). Seed PRD di repoDir project. `repoDir`/`p1` sudah ada di setup file (lihat blok prd). Tambahkan:
...
- [x] **Step 3: Edit `server/src/routes/terminal.ts`** — impor tambahan. Ubah baris import runner:
```ts
...
- [x] **Step 5: Typecheck server + commit**
...
- [x] **Step 1: Write the failing test** — tambah case di `src/test/api-client.test.ts` (ikuti pola mock fetch yang ada di file). Jika file memakai `vi.spyOn(global, "fetch")`, tambah:
...
- [x] **Step 3: Edit `src/src/api/client.ts`** — impor tipe (tambah `BreakdownDoc`, `BreakdownItem` ke daftar impor dari `@hanoman/shared` di baris 1), lalu tambah method setelah `startPrd` (baris ~195):
```ts
...
- [x] **Step 1: Edit `src/src/screens/PrdScreen.tsx`** — perluas `PrdPreviewPane` menjadi memuat manifest & menampilkan panel review. Ganti komponen `PrdPreviewPane` (baris 57–87) dengan versi yang menerima dua callback baru dari props screen. Tambah impor `BreakdownItem` dari `../api/client` (re-export) atau `@hanoman/shared`.
...
- [x] **Step 2: Edit `src/src/App.tsx`** — tambah handler dan teruskan ke `PrdScreen`. Setelah `takeToBacklog` (baris ~668) tambah:
```tsx
...
- [x] **Step 1: Create `internal/docs/adr/0069-breakdown-prd-ke-backlog-paralel.md`:**
...
- [x] **Step 2: Edit `internal/docs/README.md`** — tambah baris di bagian `## adr` (paling atas daftar):
```markdown
...
- [x] **Step 3: Edit `internal/docs/architecture/api-contract.md`** — tambah 3 endpoint (di bagian specs & terminal & docs sesuai struktur file; cari heading yang relevan). Sisipkan:
```markdown
...
- [x] **Step 4: Edit `internal/docs/architecture/data-model.md`** — tambah catatan (di bagian PRD/dokumen atau catatan turunan):
```markdown
...
- [x] **Step 5: Edit `internal/docs/entrypoints/prd.md`** — tambah satu kalimat di hilir alur PRD:
```markdown
...
- [x] **Step 6: Verify docs index integrity + commit**
...
- [x] **Step 1: Full test suite hijau**
...
- [x] **Step 2: Typecheck semua paket**

Run: `pnpm typecheck`
Expected: exit 0.

- [x] **Step 3: Live curl lokal (WAJIB per CLAUDE.md)** — boot server terhadap DB throwaway ter-migrate (bukan `hanoman_test`, hindari sibling truncation — lihat memori). Seed satu project + manifest fixture, lalu:
...
- [x] **Step 4: Centang semua kotak plan** — pastikan tiap `- [x]` di plan ini jadi `- [x]`. hanoman menahan backlog di `executing` selama masih ada `- [x]`.

- [x] **Step 5: Final commit (bila ada sisa) + siap push**
