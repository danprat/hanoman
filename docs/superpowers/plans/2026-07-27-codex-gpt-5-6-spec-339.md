...
- [ ] **Step 1: Buat worktree terisolasi**
...
- [ ] **Step 2: Siapkan DB test khusus SPEC-339**
...
- [ ] **Step 3: Pastikan baseline hijau sebelum menyentuh apa pun**
...
Run: `env -u NODE_ENV -u DATABASE_URL pnpm vitest run --no-file-parallelism shared/src/codex-catalog.test.ts`
Expected: FAIL — `codexEfforts is not a function` / `coerceCodexEffort is not exported`.

- [ ] **Step 3: Ganti blok katalog codex di `shared/src/entities.ts`**
...
- [ ] **Step 5: Perbarui test SPEC-338 yang kini memegang nilai lama**
...
- [ ] **Step 6: Jalankan seluruh test `shared` + typecheck**

Run: `env -u NODE_ENV -u DATABASE_URL pnpm vitest run --no-file-parallelism shared && pnpm --filter ./shared typecheck`
...
- [ ] **Step 6: Verifikasi endpoint hidup lewat test route**
...
Run: `env -u NODE_ENV -u DATABASE_URL pnpm vitest run --no-file-parallelism src/test/start-session-agent.test.tsx src/test/settings-agent.test.tsx`
Expected: FAIL — daftar effort masih `["xhigh","high","medium","low"]` untuk semua model.

- [ ] **Step 3: Implementasi di `StartSessionModal`**
...
- [ ] **Step 4: Implementasi di `SettingsScreen`**
...
- [ ] **Step 6: Tulis test untuk model di luar katalog**
...
Run: `env -u NODE_ENV -u DATABASE_URL pnpm vitest run --no-file-parallelism src/test/settings-agent.test.tsx`
Expected: FAIL — Select tak memuat opsi `gpt-7-belum-ada`.

- [ ] **Step 8: Implementasi opsi luar katalog**
...
Run: `env -u NODE_ENV -u DATABASE_URL pnpm vitest run --no-file-parallelism src/test/codex-version-note.test.tsx`
Expected: FAIL — `codex-version-note` tak ditemukan.

- [ ] **Step 3: Implementasi catatan**
...
- [ ] **Step 5: Perbaiki mock yang kini kurang lengkap**
...
- [ ] **Step 3: Typecheck seluruh workspace**

Run: `pnpm typecheck`
Expected: exit 0, tanpa error.

- [ ] **Step 4: Smoke API nyata (wajib per CLAUDE.md)**
