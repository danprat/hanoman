...
- Bagian: Overview, Projects (list + pagination + cari), Backlog (filter project + tab + aksi per spec + detail spec via modal: judul, stage bar, objective, field brief/QA), Runs (list + detail: pipeline, worktree, kendali, terminal), Docs (tree + preview markdown/kode + edit + hapus file asli via `DELETE /docs/*`), Triggers, Settings (model per step).
