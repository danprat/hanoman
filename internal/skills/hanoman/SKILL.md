...
- Mesin eksekusi nyata = **`server/src/services/pty.ts`**: `createSession()` men-spawn `claude <prompt> --model … --effort … --dangerously-skip-permissions --settings <guard>` di window tmux; node-pty `tmux attach` menjembatani ke WebSocket, poll 500 ms mengawasi exit + perubahan phase-file lalu broadcast frame. **tmux adalah satu-satunya sumber kebenaran pekerjaan berjalan — tidak ada baris `Run` di DB.**
...
- Sesi berjalan di worktree sendiri di `<repoDir>/.worktrees/<id>`, dibuat `--detach` dari `branchFrom` (default `main`); `baseSha` dicatat untuk rentang review (ADR-0030). Jenis sesi: **spec-flow** (feature/qa), **reverse** (project-level), **prd**, **plain terminal**, **integrate-conflict** (merge-<id>), **vps**.
...
- **Model & effort per fase** dari Settings (`phaseModels`, SPEC-238/ADR-0057): map `flow → phase → {model?,effort?}`, sel kosong → default global (`claude-opus-4-8` / `xhigh`). Sesi tetap **satu proses** — lahir dengan config fase pertama, fase berikutnya diganti agen via `/model`+`/effort` (aman terhadap konteks; `/effort` best-effort di Opus/Fable). Manusia tetap bisa `/model` manual di terminal. `steps` headless (ADR-0003) tetap usang; mekanisme baru in-session, bukan respawn.
...
- **Alur fitur:** spec → plan → execute. **Alur QA:** audit → keputusan → (spec → plan)? → execute — temuan kecil langsung execute, Spec & Plan ditandai `skipped`; keputusan dielicit lewat prompt & diambil agen (ADR-0020/0040).
