...
- Sesi berjalan di worktree sendiri di `<repoDir>/.worktrees/<id>`, dibuat `--detach` dari `branchFrom` (default `main`); `baseSha` dicatat untuk rentang review (ADR-0030). Jenis sesi: **spec-flow** (feature/qa), **reverse** (project-level), **prd**, **plain terminal** (claude di repoDir; atau shell mentah non-claude via `{shell:true}`, SPEC-236/ADR-0056), **integrate-conflict** (merge-<id>), **vps**.
...
- **Satu** `--model`/`--effort` per sesi dari Settings (default `claude-opus-4-8` / `xhigh`); manusia bisa `/model` di dalam terminal. Model-per-step (ADR-0003) usang bersama runner headless (ADR-0024).
