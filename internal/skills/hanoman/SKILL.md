...
- **Model & effort per fase** dari Settings (`phaseModels`, SPEC-238/ADR-0058): map `flow → phase → {model?,effort?}`, sel kosong → default global (`claude-opus-4-8` / `xhigh`). Sesi tetap **satu proses** — lahir dengan config fase pertama, fase berikutnya diganti agen via `/model`+`/effort` (aman terhadap konteks; `/effort` best-effort di Opus/Fable). Manusia tetap bisa `/model` manual di terminal. `steps` headless (ADR-0003) tetap usang; mekanisme baru in-session, bukan respawn.
