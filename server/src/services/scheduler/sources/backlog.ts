...
import { enqueue, UNSTARTED_SPEC_WHERE } from "../queue";

// SPEC-295 · daun #1 scheduler otonom (di atas fondasi SPEC-294/ADR-0072). Checker "backlog":
// enqueue semua Spec belum-mulai dari project schedulerOptIn, urut prioritas tinggi→sedang→rendah.
// Idempotensi ditanggung enqueue (upsert specId @unique) — checker tetap thin, tak perlu dedup
// manual. PRD §Source — Backlog + User Story #1.
...
export async function checkBacklog(): Promise<void> {
  ...
  // SPEC-431 · "belum-mulai" = `UNSTARTED_SPEC_WHERE`, BUKAN `baseSha:null` telanjang — predikat lama
  // memasukkan item yang sudah `done` dan meluncurkannya lagi sebagai sesi tmux sungguhan.
