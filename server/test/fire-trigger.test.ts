...
describe("fireTrigger", () => {
  ...
  // SPEC-143. fireTrigger menyusun SATU objek `base` ber-branchFrom lalu menyebarnya ke setiap
  // spec dalam fan-out. Tanpa override di dalam loop, memperbaiki POST /runs saja membuat tombol
  // "Mulai" bekerja sementara run dari trigger diam-diam tetap di `main`.
  const t = (over: Record<string, unknown>) =>
    ({ id: "t", projectId: "p1", detail: "d", target: "plan + execute", enabled: true, ...over }) as any;

  it("commit trigger: ctx.branch beats spec.branchFrom", async () => {
    await prisma.spec.update({ where: { id: "SPEC-1" }, data: { branchFrom: "dev" } });
    const spy = vi.spyOn(queue, "enqueueRun").mockResolvedValue({ enqueued: true });
    await fireTrigger(t({ type: "commit" }), { branch: "release/v2" });
    expect(spy.mock.calls[0]![0]).toMatchObject({ branchFrom: "release/v2" });
  });
  it("schedule trigger: spec.branchFrom wins", async () => {
    await prisma.spec.update({ where: { id: "SPEC-1" }, data: { branchFrom: "dev" } });
    const spy = vi.spyOn(queue, "enqueueRun").mockResolvedValue({ enqueued: true });
    await fireTrigger(t({ type: "schedule" }));
    expect(spy.mock.calls[0]![0]).toMatchObject({ branchFrom: "dev" });
  });
  it("a spec with no branch falls back to main", async () => {
    const spy = vi.spyOn(queue, "enqueueRun").mockResolvedValue({ enqueued: true });
    await fireTrigger(t({ type: "manual" }));
