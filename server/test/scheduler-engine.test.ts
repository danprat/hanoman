...
const noLaunch: GovernorDeps = { liveCount: () => 0, isLive: () => null, isDone: async () => false, launch: async () => "s" };
...
describe("engine.tick gating", () => {
  ...
  it("Pause blocks launches within one tick (checkers may run, drain does not)", async () => {
    ...
    await enqueue({ specId: "SPEC-1", projectId: "p1", source: "backlog", priority: "tinggi" });
    let launches = 0;
    await tick(1_000_000, { liveCount: () => 0, isLive: () => null, isDone: async () => false, launch: async () => { launches++; return "s"; } });
  ...
  it("un-paused, the next tick drains the queued item", async () => {
    ...
    await enqueue({ specId: "SPEC-1", projectId: "p1", source: "backlog", priority: "tinggi" });
    let launches = 0;
    await tick(1_000_000, { liveCount: () => 0, isLive: () => null, isDone: async () => false, launch: async () => { launches++; return "s1"; } });
